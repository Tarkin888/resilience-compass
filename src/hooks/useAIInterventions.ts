...
async function fetchInterventions(
  ...
  score: number,
  ragBand: string,
  parsedDps: DataPointInfo[],
): Promise<TieredIntervention[]> {
...
export function useAIInterventions({
  ...
  const enabled = score != null && !!ragBand;

  const query = useQuery({
    queryKey: ["ai-interventions", pillarName, score, ragBand, dpKey],
    queryFn: () =>
      fetchInterventions(pillarName, score as number, ragBand as string, JSON.parse(dpKey)),
    enabled,
