...
export const TrendPanel = () => {
  // Dashboard-level series. Only periods where at least one scored
  // entity has data make it into score_history, so empty = no history yet.
  ...
  const spc = useMemo(
    () => classifyTrend(series.map((s) => s.score)),
    [series],
  );
